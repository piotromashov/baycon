...
def run_generator(model, random_alternatives, dataconstraints, initial_instance, target):    
    ...
    while epoch_counter < epochs_threshold and best_global_no_improvement_counter < best_global_no_improvement_threshold:
        ...
        if not len(iteration_instances) or not len(scores_achieved_target):
            distance_from_best_to_initial_instance = npu.distance(np.array(best_global_instance),np.array(initial_instance))
        ...
        print("Known alternatives size {}".format(len(known_instances)))
        print("Promising alternatives: {}".format(len(promising_alternatives_pool)))
        print("Best instance: {}, score {}, found in epoch: {}".format(best_global_instance, best_global_score, epoch_max_found))
        
        #retrain surrogate model with updated training data
        #TODO: improvement: do this if there has been an % increase in data
        print("re-training surrogate model with data size: {}".format(X.shape))
        checker.train_surrogate(X, Y)
